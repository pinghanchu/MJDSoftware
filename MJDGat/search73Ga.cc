...
int main(int argc, char** argv)
{
  ...
  if(fin.is_open()){
    while(!fin.eof()){
      fin >> run1 >> entry1 >> channel1 >> enr1 >> time1 >> entry2 >> channel2 >> enr2 >> time2;
      Run1.push_back(run1);
  ...
  if(Index.size()>0){    
    ofstream fout(Form("wf_%d_%d_%d.txt",fRun,(Int_t)fEnergy,fTimems),ios::app);
    fout.precision(3);
