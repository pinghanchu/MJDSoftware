...
TH1D* CleanHisto(TH1D* Input, Double_t Mean){
  ...
  for(Int_t ib=0;ib<nbin;ib++){
    if((Input->GetBinCenter(ib)>Mean*(1-0.05) && Input->GetBinCenter(ib)<Mean*(1-0.01))){
      h->SetBinContent(ib,Input->GetBinContent(ib-500));
    }
    if((Input->GetBinCenter(ib)>Mean*(1+0.01) && Input->GetBinCenter(ib)<Mean*(1+0.05))){
...
int main(int argc, char** argv)
{
  ...
  for(size_t i = 0;i<index.size();i++){
	...
	for(size_t ip = 0;ip<fPeak.size();ip++){
	  ...
	  Par.clear();
	  ParErr.clear();
	  ds.EzFit(h3, fPeak.at(ip), 5, rebin, &Par,&ParErr, "", PlotName,TitleName);
	...
	if(fEName == "trapENF"){
	  ...
	  if(h3->GetRMS()<2.5 && h3->GetEffectiveEntries()>10){
	    Par.clear();
	    ParErr.clear();
