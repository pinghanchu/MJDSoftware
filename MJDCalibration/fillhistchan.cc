...
int main(int argc, char** argv)
{
  if(argc != 10 || atoi(argv[1]) == 0) {
    cout << "Usage: " << argv[0] << " [run] [energy name] [Bin HG] [Low HG] [Up HG] [Bin LG] [Low LG] [Up LG] [channel]" << endl;
    return 1;
  }
  ...
  Int_t fChan = atoi(argv[9]);

  string FileName = Form("./Hist/hist_%d.root",fRun);  
  ...
  for(size_t i=0;i<index.size();i++){
    ...
    if((fRun >= 9913 && fRun <= 9926) || (fRun >= 13071 && fRun <= 13074)){
      ...
      //}else if(fEName == "trapENFBL" || (found>0 && found < 10)){
      //cut1 = Form("channel == %d && trapENF>200 && RawWFblSlope>-0.0002 && RawWFblSlope<0.0002",fChannel.at(ii));
    }else{
      cut1 = Form("channel == %d",fChannel.at(ii));
    }
