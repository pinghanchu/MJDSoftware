...
0. Copy this package to your home on PDSF and then "make" to compile the code. Make "MkCookie" is linked to the latest software (the current one is at "/global/project/projectdirs/majorana/software/sl64/mjsw/mjsw201704Prod/MGDO/bin/"). If the link does not work, please contact with Robert Varnerrl.
...
This will run the script "calibration.pl" and do calibration. This script will submit jobs for the energy [energy name] = trapE, trapENF, trapENM of each detector. 
a. Check if $dataset is the correct path for the GAT data.
b. Comment out Step 2 and Step 3.
c. Check if all jobs are done. The path saving the output text files is "./List/[startrun]_[endrun]/". It includes "calibration_[startrun]_[endrun]_*.txt", "cov_[startrun]_[endrun]_*.txt", and "reso_[startrun]_[endrun]_*.txt". Qsub doesn't always finish jobs. After all jobs are done, re-run "submit.pl". Repeat this process few times until no job will be submitted. You can also use "repeat.pl" to repeat this process every 10 mins. Remeber to change the username in "repeat.pl".

4. Run teh Step 2 of the script "submit.pl".
This will generate a document "calibration_[start run]_[end run]_[energy name].pdf". It includes plots of fitted spectrum and the deviation of the linear fitting. 
a. Comment out Step 1 and Step 3.
b. Check if the peak fitting is reasonable and the deviation is not too big from "calibration_[start run]_[end run]_[energy name].pdf". 
